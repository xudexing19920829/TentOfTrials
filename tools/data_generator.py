...
def random_datetime(rng: random.Random = None, start_year: int = 2023, end_year: int = 2024) -> datetime:
    if rng is None:
        rng = random.Random()
...
def validate_count(value):
    ...
    val = int(value)
    if val < 0:
        raise argparse.ArgumentTypeError(f"count must be non-negative, got {val}")
    return val
...
def parse_args():
    ...
    parser.add_argument("--users", type=validate_count, default=50, help="Number of users to generate")
    parser.add_argument("--orders", type=validate_count, default=200, help="Number of orders to generate")
    parser.add_argument("--trades", type=validate_count, default=500, help="Number of trades to generate")
    parser.add_argument("--ticks", type=validate_count, default=1000, help="Number of ticks per instrument")
    parser.add_argument("--candles", type=validate_count, default=500, help="Number of candles per instrument")
...
def main():
    ...
    # FIX: Handle --json and --csv flags as shortcuts
    ...
    # FIX: --format both now properly emits BOTH json and csv
